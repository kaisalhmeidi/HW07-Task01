...
public class ConcreteVisitor implements Visitor {
	...
	public void cost(Electronic electronic) {
		...
		if (electronic.getFragile() == true) {
			cost = 5*(electronic.getWeight()/10);
			System.out.println("Total Cost for " + electronic.getName() + " is: " + cost);
		}else {
			cost = 5*(electronic.getWeight()/15);
			System.out.println("Total Cost for " + electronic.getName() + " is: " + cost);
		}
	}
	...
	public void cost(Furniture furniture) {
		double cost = 5*(furniture.getWeight()/20);
	...
	public void packing(Glass glass) {
		...
		System.out.println("    >should be filled with Bubble wraps and packed in a box with a dimension : " + (glass.getLenght()+1) + "x" + (glass.getWidth()+1) + "x" + (glass.getHeight()+1));
	...
	public void packing(Electronic electronic) {
		...
		System.out.println("    >should be covered with Polyethylene foam film and packed in a box with a dimension : " + (electronic.getLenght()+1) + "x" + (electronic.getWidth()+1) + "x" + (electronic.getHeight()+1));	
	}

	@Override
	public void packing(Furniture furniture) {
		System.out.println(furniture.getName() + ":");
		System.out.println("    >should be covered with waterproof covers with area of : " + (furniture.getLenght())+ "x" + (furniture.getWidth()) + "x" + (furniture.getHeight()));
